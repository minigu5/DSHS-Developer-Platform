...
| **Step 1** | Next.js (App Router) 초기 세팅, Tailwind + shadcn/ui 설치, 디렉토리 구조 정리 | ⏳ **진행 예정** |
| **Step 2** | Supabase 클라이언트(`@supabase/ssr`) 구성, Google 로그인 구현, `@ts.hs.kr` 도메인 검증 Middleware | ⏳ |
| **Step 3** | `users` / `projects` / `reviews` 테이블 + RLS SQL 작성 (Supabase SQL Editor 실행) | ⏳ |
| **Step 4** | 프로젝트 게시 폼(Form) UI + Supabase Insert 로직 | ⏳ |
| **Step 5** | 메인/탐색(Explore) 페이지 + 다중 조건 필터링 | ⏳ |
